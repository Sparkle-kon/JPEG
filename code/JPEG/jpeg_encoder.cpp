...
#pragma pack(pop)
	...
	do
	{
		...
		if((width&7)!=0 || (height&7)!=0) break;	//������8�ı���
...
//-------------------------------------------------------------------------------
void JpegEncoder::_initQualityTables(int quality_scale) //之字形排序
{
...
//-------------------------------------------------------------------------------
void JpegEncoder::_convertColorSpace(int xPos, int yPos, char* yData, char* cbData, char* crData) //RGB转换为YCbCr
{
	for (int y=0; y<8; y++)
	{
...
//-------------------------------------------------------------------------------
void JpegEncoder::_foword_FDC(const char* channel_data, short* fdc_data)  //二维DCT转换
{
	...
	for(int v=0; v<8; v++)
	{
		for(int u=0; u<8; u++)
		{
			...
			float alpha_v = (v==0) ? 1/sqrt(8.0f) : 0.5f;//计算alpha(u\v)的值

			float temp = 0.f;
			for(int x=0; x<8; x++)//计算求和
			{
				for(int y=0; y<8; y++)
				{
					float data = channel_data[y*8+x];//二维数组化为一维数组的方式输出，定义一个中间变量

					data *= cos((2*x+1)*u*PI/16.0f); 
					data *= cos((2*y+1)*v*PI/16.0f); //二维DCT转换公式中后半部分
